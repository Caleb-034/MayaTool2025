...
from MayaUtils import QMayaWindow#creates basic maya window
...
class LimbRigger: #class that sets the rigging of limbs with FK cntrls
    ...
    def RigLimb(self, r, g, b):#creates 3 paretned FK controllers for all joints
...
class LimbRigToolWidget(QMayaWindow):  #creates widgets that are in LimbRigWindow
    ...
    def RigLimbBtnClicked(self): #riglimb function
        self.rigger.RigLimb(self.colorPicker.color.redF(), self.colorPicker.color.greenF(), self.colorPicker.color.blueF())

    def AutoFindBtnClicked(self): #finds/sets joints and gives an error message if selected incorrectly
